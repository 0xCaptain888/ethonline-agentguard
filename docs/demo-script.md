...
Keep the public Marketplace visible. Use no terminal until the final integrity
command; the story should feel like a product, not a code tour.
...
| 2:48–3:00 | Evidence + boundary | “Every claim is replayable in the browser or with one command. This is Testnet software: mutual recovery is live; audit, quorum arbitration and Mainnet are next.” |

## One-sentence Monad answer

> Monad makes fine-grained Agent authority practical, and its parallel execution
> model directly shaped our move from a measured global-counter bottleneck to
> deployed per-buyer settlement lanes.

## Recording rules

- Call Task 56, Tasks 26/27, and both benchmarks `LIVE_TESTNET` evidence.
- Call the adjustable policy playground `SIMULATION`.
- Do not call application measurements Monad TPS.
- V1 and V2 may both be described as Sourcify exact-match verified.
- Do not claim completed external users until a third-party wallet submits a pilot.
