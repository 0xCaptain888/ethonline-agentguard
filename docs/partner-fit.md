...
- **Hedera:** excellent thematic fit, but the AI-payment prize requires a live
  x402 service settled through Blocky402 on Hedera. Adding a second chain and a
  second facilitator would be the largest integration surface.
- **World:** compelling identity angle, but AgentKit plus Sandbox App states
  and feedback documentation would make human-backed identity the main story,
  not policy-gated commerce.
- **1inch:** the highest upside requires a real Aqua/SwapVM DeFi position and
  on-chain token transfers. That is a new DeFi product, not an extension of
...
The event judges technicality, originality, practicality, usability/DX, and
WOW factor. Our winning narrative is therefore one observable loop, not three
unrelated integrations:

`live Graph signal → bounded policy decision → USDC escrow on Arc → independent
verification → receipt → reusable Bazantic recipe`
