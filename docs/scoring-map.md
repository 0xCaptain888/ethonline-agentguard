...
| --- | --- | --- | --- |
| Technicality | Graph data, policy evaluation, escrow state machine and independent verification compose into one flow | `src/ethonline/graph-agent.ts`, `contracts/PolicyEscrowERC20.sol`, tests | Graph observation → policy hash → verifier |
...
| Usability / DX | A judge can reproduce checks without credentials and inspect every status label | `docs/judge-guide.md`, `scripts/verify-ethonline-manifest.ts` | `npm run ethonline:check` |
