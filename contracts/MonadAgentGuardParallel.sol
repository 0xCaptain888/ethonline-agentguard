...
/// @title MonadAgentGuardParallel
/// @notice Parallel-safe AgentGuard task escrow for independent buyer lanes.
/// @dev Task ids are derived from a per-buyer nonce, avoiding the global
///      nextTaskId write hotspot in the reference V1 deployment.
contract MonadAgentGuardParallel {
    ...
    event TaskCreated(uint256 indexed taskId, address indexed buyer, address indexed seller, uint256 buyerNonce, uint256 value, bytes32 intentHash, bytes32 policyHash);
    ...
    function createTask(address seller, bytes32 intentHash, bytes32 policyHash) external payable returns (uint256 taskId) {
        ...
        uint256 buyerNonce = nextBuyerNonce[msg.sender]++;
        taskId = uint256(keccak256(abi.encodePacked(block.chainid, address(this), msg.sender, buyerNonce)));
        ...
        emit TaskCreated(taskId, msg.sender, seller, buyerNonce, msg.value, intentHash, policyHash);
    }
